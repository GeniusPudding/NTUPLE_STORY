...
class EndingScreen(Screen):
    def __init__(self, **kwargs):
        ...
        print('end self.size:',self.size)
        self.label = Label(text='ABCDEFGHIJKLMNOP',font_size=100,pos=(.25*self.size[0],.1*self.size[1]),size=(.5*self.size[0],.2*self.size[1]))
    ...
    def key_action(self, *args):
        if self.manager.current == 'ending': 
            ...
            if press_key_id in [274,273]:#<-,->     
                self.animate(self.pos,press_key_id)

    def animate(self,pos,direction,duration=.35):
        (px,py) = pos
        (ox,oy) = (0,30)
